      ...
      if constexpr ( str[POS + 1] == '{' ) {
        ...
        if constexpr ( arg_id_result.arg_id.kind == arg_id_kind::index ) {
          ...
          if constexpr ( arg_index != invalid_arg_index ) {
            constexpr auto next_id = ID != manual_indexing_id ? ID + 1 : manual_indexing_id;
            return parse_replacement_field_then_tail<decltype( get_type<arg_index, Args>::value ), Args, arg_id_end_pos,
...
format_to_n_result<OutputIt> format_to_n( OutputIt out, size_t n, const S& format_str, Args&&... args ) {
  auto it =
      fmt::format_to( detail::truncating_iterator<OutputIt>( out, n ), format_str, std::forward<Args>( args )... );
