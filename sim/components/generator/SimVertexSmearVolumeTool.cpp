...
namespace megat {
  ...
  public:
    ...
    StatusCode smearVertex( CLHEP::Hep3Vector& aVertex );

  private:
    dd4hep::Solid m_solid; // solid node
    TGeoHMatrix   m_trans; // global transform
  ...
  StatusCode SimVertexSmearVolumeTool::initialize() {
    ...
    auto _geo_svc = service<IGeoSvc>( "MegatGeoSvc", true );
    if ( !_geo_svc ) {
    ...
    auto _rdm_svc = service<IRndmGenSvc>( "RndmGenSvc", true );
    if ( !_rdm_svc ) {
    ...
    m_rndmX.initialize( _rdm_svc, Rndm::Flat( -_x, _x ) );
    m_rndmY.initialize( _rdm_svc, Rndm::Flat( -_y, _y ) );
    m_rndmZ.initialize( _rdm_svc, Rndm::Flat( -_z, _z ) );

    return StatusCode::SUCCESS;
  }

  StatusCode SimVertexSmearVolumeTool::smearVertex( CLHEP::Hep3Vector& aVertex ) {
    ...
    aVertex += CLHEP::Hep3Vector( _global[0], _global[1], _global[2] );
