...
public:
  ...
  int64_t rotl( const std::uint64_t x, int k ) { return ( x << k ) | ( x >> ( 64 - k ) ); };
...
public:
  ...
  EcoMug()
      ...
      , mMaximumPhi( 2. * M_PI )
      , mCharge( 1 )
      , mHorizontalRate( 129 * EMUnits::hertz / EMUnits::m2 )
  ...
  void SetHorizontalRate( double rate ) { mHorizontalRate = rate; };
  ...
  double GetHorizontalRate() const { return mHorizontalRate; };
  ...
  double GetGenSurfaceArea() const {
    ...
    if ( mGenMethod == Sky ) {
      ...
      area = ( mHSphereMaxPositionPhi - mHSphereMinPositionPhi ) * pow( mHSphereRadius, 2 ) *
             ( cos( mHSphereMinPositionTheta ) - cos( mHSphereMaxPositionTheta ) );
    }
    return area;
  };
  ...
  void   SetSkyCenterPosition( const std::array<double, 3>& position ) { mSkyCenterPosition = position; };
  double GetSkySize( unsigned int index ) { return mSkySize[index]; };
  ///////////////////////////////////////////////////////////////
  ...
  void SetCylinderCenterPosition( const std::array<double, 3>& position ) { mCylinderCenterPosition = position; };
  ...
  const std::array<double, 3>& GetCylinderCenterPosition() const { return mCylinderCenterPosition; };
  ///////////////////////////////////////////////////////////////
  ...
  void SetHSphereCenterPosition( const std::array<double, 3>& position ) { mHSphereCenterPosition = position; };
  ...
  const std::array<double, 3>& GetHSphereCenterPosition() const { return mHSphereCenterPosition; };
  ///////////////////////////////////////////////////////////////

private:
  double F1Cumulative( double x ) { return 1. - 8.534790171171021 / pow( x + 2.68, 87. / 40. ); };

  double F1Inverse( double x ) { return ( 2.68 - 2.68 * pow( 1. - x, 40. / 87. ) ) / pow( 1. - x, 40. / 87. ); };

  double maxSkyJFunc() { return 1600 * pow( mMaximumMomentum, 0.279 ) * pow( cos( 0.76158 ), 1.1 ) * sin( 0.76158 ); };
  ...
  void GeneratePositionCylinder() {
    ...
    mGenerationPosition[2] = mRandom.GenerateRandomDouble( mCylinderCenterPosition[2] - mCylinderHeight / 2.,
  ...
  void ComputeMaximumCustomJ() {
    ...
    if ( mGenMethod == 0 || mGenMethod == 1 ) {
      ...
      maximizer.SetParameters( mMinimumMomentum, mMaximumMomentum, mMinimumTheta, mMaximumTheta, mMinimumPhi,
                               mMaximumPhi );
    }
  ...
  void MCJprimeCustomSkyIntegration( double& rate, double& error, int npoints ) {
    ...
    double V =
        ( mMaximumMomentum - mMinimumMomentum ) * ( mMaximumTheta - mMinimumTheta ) * ( mMaximumPhi - mMinimumPhi );
  ...
  void MCJprimeCustomHSphereIntegration( double& rate, double& error, int npoints ) {
    ...
    for ( auto i = 0; i < npoints; ++i ) {
      ...
      value               = mJ( mGenerationMomentum, mGenerationTheta ) *
              ( sin( mTheta0 ) * sin( mGenerationTheta ) * sin( mGenerationTheta ) * cos( mGenerationPhi ) +
    ...
    double V = ( mMaximumMomentum - mMinimumMomentum ) * ( mMaximumTheta - mMinimumTheta ) *
               ( mMaximumPhi - mMinimumPhi ) * ( mHSphereMaxPositionTheta - mHSphereMinPositionTheta );
    double expected       = I / npoints;
    double expectedSquare = I2 / npoints;
    rate                  = V * I / npoints;
    error                 = V * pow( ( expectedSquare - pow( expected, 2 ) ) / ( npoints - 1 ), 0.5 );
  ...
  void MCJprimeSkyIntegration( double& rate, double& error, int npoints ) {
    ...
    for ( auto i = 0; i < npoints; ++i ) {
      ...
      if ( mN < 0.1 ) mN = 0.1;
      value = 1600 * pow( mGenerationMomentum + 2.68, -3.175 ) * pow( mGenerationMomentum, 0.279 ) *
              pow( cos( mGenerationTheta ), mN + 1 ) * sin( mGenerationTheta );
      I += value;
      I2 += pow( value, 2 );
    }
    double V =
  ...
  void MCJprimeCylinderIntegration( double& rate, double& error, int npoints ) {
    ...
    for ( auto i = 0; i < npoints; ++i ) {
      ...
      if ( mN < 0.1 ) mN = 0.1;
      value = 1600 * pow( mGenerationMomentum + 2.68, -3.175 ) * pow( mGenerationMomentum, 0.279 ) *
              pow( cos( mGenerationTheta ), mN ) * pow( sin( mGenerationTheta ), 2 ) * cos( mGenerationPhi );
      if ( value < 0 ) value = 0;
      I += value;
      I2 += pow( value, 2 );
    }
  ...
  void GenerateFromCustomJ() {
    ...
    if ( mGenMethod == Sky || mGenMethod == Cylinder ) {
      ...
      while ( !mAccepted ) {
        ...
        if ( mGenMethod == Cylinder ) {
          mJPrime =
              mJ( mGenerationMomentum, mGenerationTheta ) * pow( sin( mGenerationTheta ), 2 ) * cos( mGenerationPhi );
          if ( mMaxCustomJ[mGenMethod] * mRandAccRej < mJPrime ) mAccepted = true;
        }
      }
      mGenerationTheta = M_PI - mGenerationTheta;
...
public:
  ...
  void SetBckWeights( const std::vector<double>& weights ) {
    if ( mBckInstances.size() != weights.size() ) {
      EMLogger( EMLog::ERROR,
                "Expected " + std::to_string( mBckInstances.size() ) + " weights, but " +
                    std::to_string( weights.size() ) + " were provided. Setting them to 1.",
                EMLog::EMMultiGen );
  ...
  void SetBckPID( const std::vector<int>& values ) {
    if ( mBckInstances.size() != values.size() ) {
      EMLogger( EMLog::ERROR,
                "Expected " + std::to_string( mBckInstances.size() ) + " PID, but " + std::to_string( values.size() ) +
                    " were provided. Setting them to 0.",
                EMLog::EMMultiGen );
  ...
  /// Get the generation position
  const std::array<double, 3>& GetGenerationPosition() const { return mBckInstances[mIndex].GetGenerationPosition(); };

  /// Get the generation momentum
  double GetGenerationMomentum() const { return mBckInstances[mIndex].GetGenerationMomentum(); };
  ...
  /// Get the generation theta
  double GetGenerationTheta() const { return mBckInstances[mIndex].GetGenerationTheta(); };

  /// Get the generation phi
  double GetGenerationPhi() const { return mBckInstances[mIndex].GetGenerationPhi(); };
