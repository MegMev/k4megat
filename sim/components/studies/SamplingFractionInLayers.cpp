...
namespace megat {
  ...
  DECLARE_COMPONENT_WITH_ID( SamplingFractionInLayers, "MegatSamplingFractionInLayers" )
  ...
  SamplingFractionInLayers::SamplingFractionInLayers( const std::string& aName, ISvcLocator* aSvcLoc )
      ...
      , m_geoSvc( "MegatGeoSvc", "SamplingFractionInLayers" )
  ...
  StatusCode SamplingFractionInLayers::initialize() {
    ...
    for ( uint i = 0; i < m_numLayers; i++ ) {
      m_totalEnLayers.push_back( new TH1F( ( "ecal_totalEnergy_layer" + std::to_string( i ) ).c_str(),
                                           ( "Total deposited energy in layer " + std::to_string( i ) ).c_str(), 1000,
                                           0, 1.2 * m_energy ) );
      ...
      m_activeEnLayers.push_back(
          new TH1F( ( "ecal_activeEnergy_layer" + std::to_string( i ) ).c_str(),
                    ( "Deposited energy in active material, in layer " + std::to_string( i ) ).c_str(), 1000, 0,
                    1.2 * m_energy ) );
  ...
  StatusCode SamplingFractionInLayers::execute() {
    ...
    if ( sumE > 0 ) { m_sf->Fill( sumEactive / sumE ); }
    for ( uint i = 0; i < m_numLayers; i++ ) {
      ...
      if ( sumElayers[i] > 0 ) { m_sfLayers[i]->Fill( sumEactiveLayers[i] / sumElayers[i] ); }
    }
    return StatusCode::SUCCESS;
  }
