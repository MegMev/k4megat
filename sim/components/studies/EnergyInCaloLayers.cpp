...
namespace megat {
  ...
  DECLARE_COMPONENT_WITH_ID( EnergyInCaloLayers, "MegatEnergyInCaloLayers" )
  ...
  StatusCode EnergyInCaloLayers::execute() {
    ...
    if ( particles->size() > 1 ) { warning() << "Found more than one initial particle!" << endmsg; }
    ...
    double rxy           = std::sqrt( std::pow( particles->at( 0 ).getMomentum().x, 2 ) +
                                      std::pow( particles->at( 0 ).getMomentum().y, 2 ) );
    ...
    for ( size_t i = 0; i < energyInLayer->size(); ++i ) { energyInLayer->at( i ) /= m_samplingFractions[i]; }

    // Energy deposited in the whole calorimeter
    double energyInCalo = 0.;
    for ( size_t i = 0; i < energyInLayer->size(); ++i ) { energyInCalo += energyInLayer->at( i ); }
    ...
    verbose() << "Energy in calorimeter and in the cryostat: " << energyInCalo + energyInCryo->at( 0 ) << " GeV"
              << endmsg;

    return StatusCode::SUCCESS;
  }

  StatusCode EnergyInCaloLayers::finalize() { return GaudiAlgorithm::finalize(); }
