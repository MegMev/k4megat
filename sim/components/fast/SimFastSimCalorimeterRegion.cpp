#include "SimFastSimCalorimeterRegion.h"
...
namespace megat {
  ...
  DECLARE_COMPONENT_WITH_ID( SimFastSimCalorimeterRegion, "SimFastSimCalorimeterRegion" )

  SimFastSimCalorimeterRegion::SimFastSimCalorimeterRegion( const std::string& type, const std::string& name,
  ...
  SimFastSimCalorimeterRegion::~SimFastSimCalorimeterRegion() {}

  StatusCode SimFastSimCalorimeterRegion::initialize() {
  ...
  StatusCode SimFastSimCalorimeterRegion::finalize() { return GaudiTool::finalize(); }

  StatusCode SimFastSimCalorimeterRegion::create() {
    ...
    for ( const auto& calorimeterName : m_volumeNames ) {
      for ( int iter_region = 0; iter_region < world->GetNoDaughters(); ++iter_region ) {
        if ( world->GetDaughter( iter_region )->GetName().find( calorimeterName ) != std::string::npos ) {
          ...
          m_parametrisation =
              std::unique_ptr<GVFlashShowerParameterisation>( m_parametrisationTool->parametrisation() );
