#include "SimFastSimHistograms.h"
...
namespace megat {
  ...
  DECLARE_COMPONENT_WITH_ID( SimFastSimHistograms, "SimFastSimHistograms" )

  SimFastSimHistograms::SimFastSimHistograms( const std::string& aName, ISvcLocator* aSvcLoc )
  ...
  SimFastSimHistograms::~SimFastSimHistograms() {}

  StatusCode SimFastSimHistograms::initialize() {
    ...
    if ( m_histSvc->regHist( "/rec/SmP", m_p ).isFailure() ) { error() << "Couldn't register SmP histogram" << endmsg; }
  ...
  StatusCode SimFastSimHistograms::execute() {
  ...
  StatusCode SimFastSimHistograms::finalize() { return GaudiAlgorithm::finalize(); }
} // namespace megat
