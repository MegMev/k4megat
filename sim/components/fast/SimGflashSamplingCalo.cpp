#include "SimGflashSamplingCalo.h"

// Geant
#include "G4NistManager.hh"
#include "GFlashSamplingShowerParameterisation.hh"

DECLARE_COMPONENT(SimGflashSamplingCalo)

SimGflashSamplingCalo::SimGflashSamplingCalo(const std::string& type, const std::string& name,
                                                 const IInterface* parent)
    : GaudiTool(type, name, parent) {}

SimGflashSamplingCalo::~SimGflashSamplingCalo() {}

StatusCode SimGflashSamplingCalo::initialize() {
...
StatusCode SimGflashSamplingCalo::finalize() { return GaudiTool::finalize(); }

std::unique_ptr<GVFlashShowerParameterisation> SimGflashSamplingCalo::parametrisation() {
  G4NistManager* nist = G4NistManager::Instance();
