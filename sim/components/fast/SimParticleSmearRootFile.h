...
namespace megat {
  ...
  /** @class SimParticleSmearRootFile SimFast/src/components/SimParticleSmearRootFile.h SimParticleSmearRootFile.h
   *
   ...
   *  'resolutions' tree has TArrayD with resolutions computed for momentum values. An array is defined for every eta
   * bin. Momentum of the particle is smeared following a Gaussian distribution, using the evaluated resolution as the
   * mean. User needs to specify the min/max momentum nad max eta for fast sim in the `SimFastSimTrackerRegion` tool.
   *  The defined values cannot be broader than eta and p values for which the resolutions were computed.
   *
   *  @author Anna Zaborowska
   */

  class SimParticleSmearRootFile : public GaudiTool, virtual public ISimParticleSmearTool {
  public:
    explicit SimParticleSmearRootFile( const std::string& type, const std::string& name, const IInterface* parent );
    virtual ~SimParticleSmearRootFile();
