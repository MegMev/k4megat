#include "SimParticleSmearRootFile.h"
...
namespace megat {
  ...
  DECLARE_COMPONENT_WITH_ID( SimParticleSmearRootFile, "SimParticleSmearRootFile" )

  SimParticleSmearRootFile::SimParticleSmearRootFile( const std::string& type, const std::string& name,
                                                      const IInterface* parent )
      : GaudiTool( type, name, parent ) {
    declareInterface<ISimParticleSmearTool>( this );
  }

  SimParticleSmearRootFile::~SimParticleSmearRootFile() {}

  StatusCode SimParticleSmearRootFile::initialize() {
  ...
  StatusCode SimParticleSmearRootFile::finalize() { return GaudiTool::finalize(); }

  StatusCode SimParticleSmearRootFile::smearMomentum( CLHEP::Hep3Vector& aMom, int /*aPdg*/ ) {
  ...
  StatusCode SimParticleSmearRootFile::readResolutions() {
    ...
    for ( int itEta = 0; itEta < binsEta; itEta++ ) {
      ...
      if ( msgLevel( MSG::DEBUG ) ) {
        debug() << "resolutions for eta (" << ( itEta == 0 ? 0 : readEta->At( itEta - 1 ) ) << ", "
                << readEta->At( itEta ) << "): \n";
  ...
  double SimParticleSmearRootFile::resolution( double aEta, double aMom ) {
    ...
    for ( auto& m : m_momentumResolutions ) {
      if ( fabs( aEta ) < m.first ) { return m.second.Eval( aMom ); }
    }
    return 0;
  }

  StatusCode SimParticleSmearRootFile::checkConditions( double aMinMomentum, double aMaxMomentum,
