// local
#include "SimConstantMagneticFieldTool.h"
...
namespace megat {
  ...
  DECLARE_COMPONENT_WITH_ID( SimConstantMagneticFieldTool, "SimConstantMagneticFieldTool" )

  SimConstantMagneticFieldTool::SimConstantMagneticFieldTool( const std::string& type, const std::string& name,
                                                              const IInterface* parent )
      : GaudiTool( type, name, parent ), m_field( nullptr ) {
    declareInterface<ISimMagneticFieldTool>( this );
  }

  SimConstantMagneticFieldTool::~SimConstantMagneticFieldTool() {
    if ( nullptr != m_field ) delete m_field;
  }

  StatusCode SimConstantMagneticFieldTool::initialize() {
  ...
  StatusCode SimConstantMagneticFieldTool::finalize() {
    StatusCode sc = GaudiTool::finalize();
    return sc;
  }

  const G4MagneticField* SimConstantMagneticFieldTool::field() const { return m_field; }

  G4MagIntegratorStepper* SimConstantMagneticFieldTool::stepper( const std::string& name,
