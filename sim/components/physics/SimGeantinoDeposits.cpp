...
namespace megat {
  ...
  G4ChargedGeantinoNegative* G4ChargedGeantinoNegative::Definition() {
    ...
    if ( anInstance == 0 ) {
      ...
      anInstance = new G4ParticleDefinition( name, 0.0 * MeV, 0.0 * MeV, -1. * eplus, 0, 0, 0, 0, 0, 0, "geantino", 0,
                                             0, 0, true, 0.0, NULL, false, "geantino", 0 );
    }
  ...
  DECLARE_COMPONENT_WITH_ID( SimGeantinoDeposits, "SimGeantinoDeposits" )

  SimGeantinoDeposits::SimGeantinoDeposits( const std::string& aType, const std::string& aName,
                                            const IInterface* aParent )
      : AlgTool( aType, aName, aParent ) {
    declareInterface<ISimPhysicsList>( this );
  }

  StatusCode SimGeantinoDeposits::initialize() { return AlgTool::initialize(); }

  StatusCode SimGeantinoDeposits::finalize() { return AlgTool::finalize(); }

  G4VModularPhysicsList* SimGeantinoDeposits::physicsList() {
