...
namespace megat {
  ...
  MaterialScan::MaterialScan( const std::string& name, ISvcLocator* svcLoc )
      : Service( name, svcLoc ), m_geoSvc( "MegatGeoSvc", name ) {}
  ...
  StatusCode MaterialScan::initialize() {
    ...
    for ( eta = -m_etaMax; eta < m_etaMax; eta += m_etaBinning ) {
      ...
      for ( int iPhi = 0; iPhi < m_nPhiTrials; ++iPhi ) {
        ...
        if ( distance == 0 ) { distance = boundaryVol->DistFromOutside( pos.data(), dir.data() ); }
  ...
  DECLARE_COMPONENT_WITH_ID( MaterialScan, "MegatMaterialScan" )
} // namespace megat
