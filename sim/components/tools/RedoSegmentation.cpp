...
namespace megat {
  ...
  DECLARE_COMPONENT_WITH_ID( RedoSegmentation, "MegatRedoSegmentation" )

  RedoSegmentation::RedoSegmentation( const std::string& aName, ISvcLocator* aSvcLoc )
      : GaudiAlgorithm( aName, aSvcLoc )
      , m_geoSvc( "MegatGeoSvc", aName )
      , m_eventDataSvc( "EventDataSvc", "RedoSegmentation" ) {
  ...
  StatusCode RedoSegmentation::initialize() {
    ...
    for ( uint itField = 0; itField < m_oldDecoder->size(); itField++ ) {
      ...
      if ( iter == m_oldIdentifiers.end() ) { m_detectorIdentifiers.push_back( field ); }
    }
  ...
  StatusCode RedoSegmentation::execute() {
    ...
    for ( const auto& hit : *inHits ) {
      ...
      if ( debugIter < m_debugPrint ) { debug() << "OLD: " << m_oldDecoder->valueString( cellId ) << endmsg; }
  ...
  uint64_t RedoSegmentation::volumeID( uint64_t aCellId ) const {
    ...
    for ( const auto& identifier : m_oldIdentifiers ) { m_oldDecoder->set( cID, identifier, 0 ); }
    return cID;
  }
} // namespace megat
