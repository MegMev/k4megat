...
namespace megat {
  ...
  class SimSaveHistory : public GaudiTool, virtual public ISimSaveOutputTool {
  public:
    explicit SimSaveHistory( const std::string& aType, const std::string& aName, const IInterface* aParent )
        : GaudiTool( aType, aName, aParent ) {
      declareInterface<ISimSaveOutputTool>( this );
      declareProperty( "Hits", m_mcParticles, "Handle to the secondary particles" );
    }

    ~SimSaveHistory() = default;
  ...
  DECLARE_COMPONENT_WITH_ID( SimSaveHistory, "SimSaveHistory" )
