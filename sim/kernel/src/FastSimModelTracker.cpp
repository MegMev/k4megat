...
namespace megat {
  ...
  namespace sim {
    ...
    G4bool FastSimModelTracker::ModelTrigger( const G4FastTrack& aFastTrack ) {
      ...
      // check the threshold
      if ( momentum >= m_minTriggerMomentum && momentum <= m_maxTriggerMomentum ) { return true; }
      return false;
    }
    ...
    void FastSimModelTracker::DoIt( const G4FastTrack& aFastTrack, G4FastStep& aFastStep ) {
      ...
      fPathFinder->ComputeStep( aFieldTrack, currentMinimumStep, 0, track->GetCurrentStepNumber(), retSafety,
                                retStepLimited, endTrack, track->GetVolume() );
      ...
      if ( track->GetParentID() == 0 ) {
        ParticleInformation* info = dynamic_cast<ParticleInformation*>(
