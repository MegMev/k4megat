...
namespace megat {
  ...
  namespace sim {
    ...
    GeoConstruction::GeoConstruction( dd4hep::Detector& lcdd, std::map<std::string, std::string> sensitive_types )
        : m_lcdd( lcdd ), m_sensitive_types( sensitive_types ) {}
    ...
    void GeoConstruction::ConstructSDandField() {
      ...
      for ( _SV::const_iterator iv = vols.begin(); iv != vols.end(); ++iv ) {
        ...
        if ( m_sensitive_types.find( typ ) != m_sensitive_types.end() ) { typ = m_sensitive_types[typ]; }
        ...
        for ( VolSet::const_iterator i = sens_vols.begin(); i != sens_vols.end(); ++i ) {
          ...
          if ( g4v == nullptr ) {
            throw std::runtime_error( "ConstructSDandField: Failed to access G4LogicalVolume for SD " + nam +
                                      " of type " + typ + "." );
          }
          G4SDManager::GetSDMpointer()->AddNewDetector( g4sd );
    ...
    G4VPhysicalVolume* GeoConstruction::Construct() {
      ...
      if ( not m_lcdd.volumeManager().isValid() ) { m_lcdd.apply( "DD4hepVolumeManager", 0, 0 ); }
