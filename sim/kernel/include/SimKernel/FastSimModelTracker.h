...
namespace megat {
  ...
  /** FastSimModelTracker SimFast/SimFast/FastSimModelTracker.h FastSimModelTracker.h
   *
   ...
   *  b) if minimum and/or maximum momentum is defined, particle within that range are parametrised. Maximum eta can
   * also be set. Volumes are represented by G4Region given to the constructor of FastSimModelTracker. For those
   * volumes, if the model is triggered, instead of the ordinary tracking, a particle is moved to the exit of the
   * tracker (as it would be transported with initial momentum and no physics processes on the way) and the particle
   * momentum is smeared according to the smearing tool defined in the job options file.
   *
  ...
  namespace sim {
    ...
    public:
       ...
       *  @param aSmearToolType Type of the implementation of ISimParticleSmearTool (conrete class name) to be searched
       * for by ToolSvc.
       *  @param aSmearToolName Name of the implementation of ISimParticleSmearTool to be searched for by ToolSvc.
      ...
      explicit FastSimModelTracker( const std::string& aModelName, G4Region* aEnvelope,
                                    ToolHandle<ISimParticleSmearTool>& aSmearTool, double aMinMomentum,
                                    double aMaxMomentum, double aMaxEta );
      /** Constructor.
       *  @param aModelName Name of the fast simulation model.
       */
      explicit FastSimModelTracker( const std::string& aModelName, ToolHandle<ISimParticleSmearTool>& aSmearTool );
    ...
    private:
      ...
      ToolHandle<ISimParticleSmearTool>& m_smearTool;
