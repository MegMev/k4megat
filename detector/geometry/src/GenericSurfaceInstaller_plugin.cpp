...
#define DD4HEP_USE_SURFACEINSTALL_HELPER Megat_GenericSurfaceInstallerPlugin
  ...
  void Installer<UserData>::install( dd4hep::DetElement component, dd4hep::PlacedVolume pv ) {
    ...
    if ( comp_vol.isSensitive() ) {
      ...
      if ( !comp_shape.isValid() || !mod_shape.isValid() ) {
        invalidInstaller(
            "Megat_GenericSurfaceInstallerPlugin: Components and/or modules are not boxes -- invalid shapes" );
