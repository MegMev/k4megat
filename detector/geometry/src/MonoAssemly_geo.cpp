...
static Ref_t create_element(Detector& description, xml_h e, SensitiveDetector sens) {
    ...
    if (x_env.hasAttr(_U(vis))) assembly.setVisAttributes(description, x_env.visStr());

    if (x_env.hasAttr(_U(region))) assembly.setRegion(description, x_env.regionStr());

    if (x_env.hasAttr(_U(limits))) assembly.setLimitSet(description, x_env.limitsStr());
    ...
    PlacedVolume pv = builder.placeDetector(assembly, (x_pos || x_rot || x_tr) ? x_env : x_det);
    if (x_det.hasAttr(_U(id))) {
        pv.addPhysVolID("system", x_det.id());
    }
    printout(builder.debug ? ALWAYS : DEBUG, "VolumeBuilder",
             "+++ Created subdetector instance %s vis:", builder.name.c_str(), x_det.visStr().c_str());
    return builder.detector;
}
DECLARE_DETELEMENT(Megat_MonoAssembly, create_element)
